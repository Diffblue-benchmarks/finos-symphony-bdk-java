...
class AppAuthSessionCertImplDiffblueTest {
   ...
   * <p>
   * Method under test: {@link AppAuthSessionCertImpl#refresh()}
   */
  @Test
  ...
  void testRefresh() throws AuthUnauthorizedException {
    ...
    when(authenticator.retrieveExtensionAppSession(Mockito.<String>any())).thenReturn(new ExtensionAppTokens());
    AppAuthSessionCertImpl appAuthSessionCertImpl = new AppAuthSessionCertImpl(authenticator, "ABC123");

    // Act
    appAuthSessionCertImpl.refresh();

    // Assert
    verify(authenticator).retrieveExtensionAppSession(eq("ABC123"));
