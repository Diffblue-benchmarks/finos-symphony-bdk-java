...
class OboAuthenticatorCertImplDiffblueTest {
  ...
  void testNewOboAuthenticatorCertImpl_whenNull_thenReturnAppIdIs42() {
    ...
    // Act
    OboAuthenticatorCertImpl actualOboAuthenticatorCertImpl =
        new OboAuthenticatorCertImpl(retryConfig, "42", null);

    // Assert
    assertEquals("42", actualOboAuthenticatorCertImpl.appId);
  }
  ...
  void testGetBasePath_thenReturnHttpsExampleOrgExample() {
    ...
    OboAuthenticatorCertImpl oboAuthenticatorCertImpl =
        new OboAuthenticatorCertImpl(retryConfig, "42", sessionAuthClient);

    // Act and Assert
    assertEquals("https://example.org/example", oboAuthenticatorCertImpl.getBasePath());
  }
}
