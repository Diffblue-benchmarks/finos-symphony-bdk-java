...
class ConnectionServiceDiffblueTest {
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"OboConnectionService ConnectionService.obo(AuthSession)"})
  void testObo_givenConnectionApiWithApiClientIsNull_thenReturnConnectionService() {
    ...
    // Act
    OboConnectionService actualOboResult =
        connectionService.obo(new AuthSessionImpl(authenticator));

    // Assert
    assertTrue(actualOboResult instanceof ConnectionService);
  }
}
