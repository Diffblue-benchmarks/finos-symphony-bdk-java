...
class ExternalLoadBalancingStrategyDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Given {@link AgentInfo} (default constructor) serverFqdn {@code agent-info}.
   *   <li>Then return {@code https://agent-info}.
   * </ul>
   *
   * <p>Method under test: {@link ExternalLoadBalancingStrategy#getNewBasePath()}
   */
  @Test
  @DisplayName(
      "Test getNewBasePath(); given AgentInfo (default constructor) serverFqdn 'agent-info'; then return 'https://agent-info'")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"String ExternalLoadBalancingStrategy.getNewBasePath()"})
  void testGetNewBasePath_givenAgentInfoServerFqdnAgentInfo_thenReturnHttpsAgentInfo()
      throws ApiException {
    // Arrange
    HashMap<String, String> defaultHeaders = new HashMap<>();
    defaultHeaders.put("Delivered-To", "alice.liddell@example.org");
    ...
    agentInfo.serverFqdn("agent-info");
    ...
    ExternalLoadBalancingStrategy externalLoadBalancingStrategy =
        new ExternalLoadBalancingStrategy(retryConfig, signalsApi);

    // Act
    String actualNewBasePath = externalLoadBalancingStrategy.getNewBasePath();

    // Assert
    verify(signalsApi).getApiClient();
    verify(signalsApi).v1InfoGet();
    assertEquals("https://agent-info", actualNewBasePath);
  }
}
