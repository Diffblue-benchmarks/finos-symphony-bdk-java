...
class AgentVersionServiceDiffblueTest {
   ...
   * </ul>
   * <p>
   * Method under test: {@link AgentVersionService#retrieveAgentVersion()}
   */
  @Test
  ...
  void testRetrieveAgentVersion_thenReturnNotPresent() {
    ...
    when(builder.get()).thenReturn(new OutboundJaxrsResponse(status, new OutboundMessageContext()));
    ...
    // Act
    Optional<AgentVersion> actualRetrieveAgentVersionResult = (new AgentVersionService(
        new SignalsApi(new ApiClientJersey2(httpClient, "https://example.org/example", defaultHeaders,
            "https://example.org/example"))))
        .retrieveAgentVersion();

    // Assert
    verify(httpClient).target(eq("https://example.org/example/v1/info"));
    verify(builder5).accept(isA(String[].class));
    verify(builder3).header(eq("Delivered-To"), isA(Object.class));
    verify(builder4).header(eq("X-Trace-Id"), isA(Object.class));
    verify(builder2).header(eq("application/json ;U"), isA(Object.class));
    verify(builder).get();
